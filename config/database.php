<?php
// config/database.php
// Kredensial dibaca dari .env — JANGAN hardcode di sini
...
$database = $_ENV['DB_DATABASE'] ?? 'annadzir_booking';
...
/**
 * Buat notifikasi untuk satu atau banyak user
 * @param mysqli $db
 * @param int|array $id_user  ID user penerima (atau array of IDs)
 * @param string $judul
 * @param string $pesan
 * @param string $tipe       info|success|warning|danger
 * @param string|null $link  URL relatif tujuan klik
 */
function kirimNotifikasi($db, $id_user, string $judul, string $pesan, string $tipe = 'info', ?string $link = null): void
{
    ...
    foreach ($targets as $uid) {
        $uid = (int) $uid;
        if ($uid > 0) {
            $db->query("INSERT INTO notifikasi (id_user, judul, pesan, tipe, link) 
                        VALUES ($uid, '$judul', '$pesan', '$tipe', $link_sql)");
        }
    }
}
